...
def execute_graph(chat_bot: List[Dict]) -> List[Dict]:
    ...
    if user_input[-1]['text'].strip().lower() != 'y':  # 正常的用户提问
    ...
    for event in events:
        ...
        if messages:
            if isinstance(messages, list):
                ...
                msg_repr = message.pretty_repr(html=True) # 调用消息对象的 pretty_repr() 方法，生成一个格式化的、易读的消息表示
...
with gr.Blocks(title='携程AI智能助手') as instance:
    ...
    chatbot = gr.Chatbot(height=350, label='AI助手')  # 聊天记录组件

    input_textbox = gr.Textbox(label='请输入你的问题📝', value='')  # 输入框组件
    # 当用户在输入框中点击提交或按Enter时触发，执行do_graph，输入为 [input_textbox, chatbot] 两个组件中的值
    # 输出：[input_textbox, chatbot] - 将函数返回值更新这两个组件
    # .then(...) - 链式执行，第一个函数完成后立即执行
