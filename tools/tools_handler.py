...
def create_tool_node_with_fallback(tools: list) -> dict:
    ...
    创建一个带有回退机制的工具节点。当指定的工具执行失败时（例如抛出异常），将触发回退操作。
        原始工具节点：ToolNode(tools)
                ↓
        添加异常处理"保护壳"：.with_fallbacks([...])
                ↓
        返回：带保护的工具节点（增强版）
    参数:
        tools (list): 工具列表。
    ...
    返回:
        dict: 带有回退机制的工具节点。
    """
    # ToolNode(tools)  # 创建一个可以执行工具列表的节点
    # .with_fallbacks(...) 为工具节点添加异常处理机制
    # [RunnableLambda(handle_tool_error)] 回退处理器列表（可以多个） RunnableLambda 将普通函数 handle_tool_error 包装成可运行的组件
    # 工具出错时，会调用handle_tool_error函数处理错误
    return ToolNode(tools).with_fallbacks(
        # 使用handle_tool_error函数作为回退处理
        [RunnableLambda(handle_tool_error)], exception_key="error"
    ) # RunnableLambda 将普通函数包装成 LangChain 的 Runnable 对象，这个对象会自动接收当前状态作为参数


def _print_event(event: dict, _printed: set, max_length=1500): # set 集合可以实现去重
    """
