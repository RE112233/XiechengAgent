...
embeddings_model = DashScopeEmbeddings(
    model="text-embedding-v2",  # 或者 "text-embedding-v1"
)
...
class VectorStoreRetriever:
    ...
    @classmethod
    def from_docs(cls, docs): # 是一个 类方法（@classmethod），用于创建类的实例，而不需要先有实例对象。
        # 从文档生成嵌入向量
        embeddings = embeddings_model.embed_documents([doc["page_content"] for doc in docs])
        vectors = embeddings
        return cls(docs, vectors) # 调用 cls() 创建实例 ← 这里等价于 VectorStoreRetriever(docs, vectors)
